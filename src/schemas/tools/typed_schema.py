...
class SearchFlightsArgs(BaseModel):
    ...
    @classmethod
    def normalize_airport(cls, v):
        if not v:
            return "SIN"
        return v.strip().upper()

    @model_validator(mode="after")
    def infer_airport(self):
        if self.direction == "arrival":
            self.airport = self.arrival_airport or self.airport

        elif self.direction == "departure":
            self.airport = self.departure_airport or self.airport
